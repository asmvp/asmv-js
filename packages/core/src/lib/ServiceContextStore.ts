...
export interface ServiceContextStore {
     ...
     */
    store: (key: string, channel: TSerializableData, state: ServiceContextSerializedState) => Promise<void>;
    /**
...
export class MemoryServiceContextStore implements ServiceContextStore {
    ...
    public async store(key: string, channel: TSerializableData, state: ServiceContextSerializedState): Promise<void> {
